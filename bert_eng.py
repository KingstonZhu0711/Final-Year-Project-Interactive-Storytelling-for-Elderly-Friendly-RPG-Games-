...
LOG_FOLDER = r"C:\Users\Zhu Jin Shun\Desktop\COMP 4913 Capstone Project\FYP Work\Work\Code\elderly-rpg-prototype\New Code\log\English"
GOLD_FOLDER = r"C:\Users\Zhu Jin Shun\Desktop\COMP 4913 Capstone Project\FYP Work\Work\Code\elderly-rpg-prototype\New Code\gold_standards\english"
...
# Find the newest English log file
log_files = [f for f in os.listdir(LOG_FOLDER) if f.startswith("story_") and f.endswith(".txt")]
if not log_files:
    print("No English log files found.")
    exit()
...
print(f"Evaluating latest English log file: {latest_log}\n")
...
# Compute BERTScore
P, R, F1 = score([story_text], [gold_standard], lang="en", verbose=True)

print("\n" + "="*70)
print("BERTScore Evaluation Report - ENGLISH VERSION")
print("="*70)
...
report_path = os.path.join(REPORT_FOLDER, f"ENG_{timestamp}_report.txt")

with open(report_path, "w", encoding="utf-8") as f:
    f.write(f"BERTScore Evaluation Report - ENGLISH VERSION - {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
...
print(f"\nEnglish report saved to: {report_path}")